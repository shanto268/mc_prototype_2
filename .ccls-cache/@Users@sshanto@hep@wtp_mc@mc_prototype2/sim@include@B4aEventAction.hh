...
inline void B4aEventAction::AddSBAR(double step, double de, int indexTray, int indexSBAR) {

    std::cout << "indexTray is " << indexTray << " and indexSBAR is " << indexSBAR << std::endl;

    int idx=indexTray*31+indexSBAR;

    if(idx>-1 && idx<124){
        edepSc8.SBAR[idx] += de;
        edepSc8.MStepBar[idx] += step; 
    }

    if(indexTray>-1 && indexTray<4){
        edepSc8.TRAY[indexTray] +=de;
        edepSc8.MStepTray[indexTray] += step; 
    }
